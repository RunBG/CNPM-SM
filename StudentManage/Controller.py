...
def get_min_age():
    res = Regulation.query.filter(Regulation.name == 'MinAge').first()
    return int(res.value)


def get_max_age():
    res = Regulation.query.filter(Regulation.name == 'MaxAge').first()
    return int(res.value)


def get_population():
    res = Regulation.query.filter(Regulation.name == 'MaxPopulation').first()
    return int(res.value)
...
def classes_append():
    for i in ['10', '11', '12']:
        ...
        constraint = Regulation.query.filter(Regulation.name == 'Class' + i + 'Amount').first()
        if constraint is not None and len(c) != constraint.value:
